...
### 🎙️ Sounds Like a Person — Feels Like a Person
...
- Riya detects when you're talking to someone else in the room and stays quiet.
...
Jev (Judgment Engine) runs fast rule-based classification before the language model. The model is only used where it genuinely adds value, keeping the conversation snappy even on modest hardware.

---

### 🪟 Live AI Brain — Transparent and Watchable

> **Every turn, on screen: what was heard → how it was judged → what was recalled → the decision → the tools that ran → what the guard blocked → what was said — with timings.**

The team console is designed so a judge or manager can watch the AI reason through a problem in real time. Nothing happens in a black box.
...
The fix is stored in the **Solvable Rulebook**. The next caller with the same problem is answered from memory without a human. The system gets smarter with every edge case.
...
- After every call or chat, the customer automatically receives a **written summary email** with the verified details (refund reference, order status, next steps).
- Switch channels mid-issue — context follows.
...
PDFs, DOCX, Markdown, CSV, JSON — all split by department, indexed in a vector store, and linked in a knowledge graph. Test what the AI would retrieve for any sentence from the team console.
...
Tiered model engine: Qwen3.5-4B on the GPU for live calls, Qwen3.8-27B on CPU/RAM for deep post-call analysis. An optional free cloud model (Groq, Gemini, OpenRouter) can be layered on for extra polish.
...
1. **Understand.** Speech → text (Gnani), language and entities (order IDs, emails) extracted, Jev judges intent, sentiment, and who the caller is addressing.
2. **Recall.** Vector index + knowledge graph return the policies, past solved cases, and team rules from the department's own rulebook slice.
3. **Decide.** Known problem, first-time bug, tool action, or escalate.
...
6. **Learn.** The ticket, fix, and human guidance become memory. The customer gets an email summary.
...
**For the customer**
- Talk to **Riya** via an animated voice orb that reacts to listening / thinking / speaking, or chat, or email.
...
- **Ticket workspace** — one-line + detailed AI summary written mid-call, conversation, tools, knowledge used, human-action panel, full timeline, context document.
...
- **Business data** — orders, payments, shipments, customers in a real SQLite DB; importable from CSV/JSON with a lookup tester.
...
**Try it in 60 seconds:** open the console, choose **Run demo → Duplicate payment**, and watch the full flow including the approval gate. Then open the customer side, pick **Lovekesh Anand**, and talk.
...
Set each customer's address in **Customers**. Riya then emails summaries and replies to incoming messages.
...
- "Learning" means recorded events and rulebook updates — no model retraining.
